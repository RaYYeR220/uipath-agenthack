...
def _build_target(kind: str, mandate: MandateSpec) -> TargetAgent:
    if kind == "mock":
        return MockTargetAgent(pii_examples=mandate.pii_examples)
    raise SystemExit(f"target '{kind}' not available in Phase 0 (only 'mock'). "
                     "UiPath target arrives in Phase 1.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    audit.add_argument("--target", default="mock", choices=["mock"])
    ...
    target = _build_target(args.target, mandate)
